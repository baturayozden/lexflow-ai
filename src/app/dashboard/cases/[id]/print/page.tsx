...
export default async function DashboardCasePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: firm } = await supabaseAdmin
    .from('firms')
    .select('name, logo_url, primary_color, address, phone, email, website')
    .eq('id', firmId)
    .single()
  ...
  const css = `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .firm-details { font-size: 11px; color: #666; margin-top: 4px; }
    ...
    .footer-firm { font-size: 11px; color: #999; }
    .footer-powered { font-size: 10px; color: #ccc; }
    .footer-powered span { color: ${primaryColor}; }
        ...
        <div className="page">
          ...
          <div className="header">
            <div>
              {firm?.logo_url ? (
                ...
                <img src={firm.logo_url as string} alt={firm.name as string} style={{ height: '40px', marginBottom: '8px' }} />
              ) : (
                <div className="firm-name">
                  <span>{(firm?.name as string)?.charAt(0) || 'L'}</span>
                  {(firm?.name as string)?.slice(1) || 'exFlow'}
                </div>
              )}
          ...
          <div className="footer">
            <div className="footer-firm">
              {firm?.name as string} · Confidential · {new Date().toLocaleDateString('en-GB')}
            </div>
            <div className="footer-powered">
              Powered by <span>LexFlow</span>
            </div>
          </div>
        </div>
      </body>
