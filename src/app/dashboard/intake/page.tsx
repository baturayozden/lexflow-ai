...
export default function IntakePage() {
  ...
  function resetForm() {
    setStep('form')
    setResult(null)
    setForm({ name: '', email: '', phone: '', dob: '', nationality: '', visaType: 'Work Visa', visaExpiry: '', caseType: 'ILR Application', description: '' })
    setCaseId('')
    setReferenceId('')
  }
      ...
      <div className="p-8 max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <button onClick={resetForm} className="text-white/40 hover:text-white text-sm transition-colors">← New Case</button>
            <span className="text-white/20">/</span>
            <span className="text-white/60 text-sm">{form.name}</span>
          </div>
          <Link href="/dashboard/cases" className="text-[#c9a84c] text-sm hover:underline">View All Cases →</Link>
        </div>
        ...
        {!!caseId && (
          <div className="bg-white/2 border border-white/10 rounded-xl p-5 flex items-center justify-between">
            <div>
              <p className="text-white font-medium text-sm">{form.name}</p>
              <p className="text-white/40 text-xs mt-0.5">{form.caseType} · {form.nationality}</p>
            </div>
            <Link href={`/dashboard/cases/${caseId}`} className="bg-[#c9a84c]/10 border border-[#c9a84c]/30 text-[#c9a84c] text-xs px-4 py-2 rounded-lg hover:bg-[#c9a84c]/20 transition-colors">
              Open Case →
            </Link>
          </div>
        )}
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-8">
        <h1 className="text-white font-bold text-xl">AI Client Intake</h1>
        <p className="text-white/40 text-sm mt-1">{firmName} · Fill in the form to generate an AI case summary</p>
      </div>
